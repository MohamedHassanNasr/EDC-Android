...
//                writeEntireCard();
    ...
    private int authToSector(int sector, byte[] key) {
        try {
            return MyApplication.app.readCardOptV2.mifareAuth(0, sector * 4, key);
    ...
    private int readBlock(int block, byte[] data) {
        try {
            return MyApplication.app.readCardOptV2.mifareReadBlock(block, data);
        } catch (RemoteException re) {
            re.printStackTrace();
            return -1;
        }
    ...
    private int writeBlock(int block, byte[] data) {
        try {
            return MyApplication.app.readCardOptV2.mifareWriteBlock(block, data);
        } catch (RemoteException re) {
            re.printStackTrace();
            return -1;
        }

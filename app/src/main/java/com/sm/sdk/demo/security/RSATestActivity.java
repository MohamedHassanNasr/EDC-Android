...
public class RSATestActivity extends BaseAppCompatActivity {
    ...
    private void generateRSAKey() {
        try {
            result.setText(null);
            //set public key index as 0x010001(65537)
            int code = MyApplication.app.securityOptV2.generateRSAKeys(0, 1, 2048, "00010001");
            if (code == 0) {
    ...
    private void getRSAKeys() {
        try {
            ...
            int len = MyApplication.app.securityOptV2.getRSAPublicKey(0, dataOut);
            if (len < 0) {
    ...
    private void rsaPublicKeyEncryptPrivateKeyDecrypt() {
        try {
            ...
            String transformation = AidlConstantsV2.Security.RSA_TRANSFORMATION_6;
            int len = MyApplication.app.securityOptV2.dataEncryptRSA(transformation, 0, data.getBytes(), dataOut);
            if (len < 0) {
            ...
            len = MyApplication.app.securityOptV2.dataDecryptRSA(transformation, 1, validOut, dataOut);
            if (len < 0) {
    ...
    private void rsaPrivateKeyEncryptPublicKeyDecrypt() {
        try {
            ...
            byte[] dataOut = new byte[2048];
            String transformation = AidlConstantsV2.Security.RSA_TRANSFORMATION_6;
            int len = MyApplication.app.securityOptV2.dataEncryptRSA(transformation, 1, data.getBytes(), dataOut);
            if (len < 0) {
            ...
            byte[] validOut = Arrays.copyOf(dataOut, len);
            String encOut = ByteUtil.bytes2HexStr(validOut);
            len = MyApplication.app.securityOptV2.dataDecryptRSA(transformation, 0, validOut, dataOut);
            if (len < 0) {
    ...
    private void rsaSigning() {
        try {
            ...
            int len = MyApplication.app.securityOptV2.signingRSA(rasSingAlg, 1, hash, dataOut);
            if (len < 0) {
    ...
    private void rsaVerifySignature() {
        try {
            ...
            int code = MyApplication.app.securityOptV2.verifySignatureRSA(rasSingAlg, pubKey, hash, signData);
    ...
    private void saveCipherTextKeyRSA() {
        try {
            ...
            int len = MyApplication.app.securityOptV2.dataEncryptRSA(Security.RSA_TRANSFORMATION_4, 0, rawKey, dataOut);
            if (len < 0) {
                LogUtil.e(TAG, "RSA private key encrypt data failed:" + len);
                return;
            }
            ...
            int code = MyApplication.app.securityOptV2.saveCiphertextKeyRSA(Security.KEY_TYPE_REC, ciphertextkey, kcv, Security.KEY_ALG_TYPE_3DES, 2,
            ...
            code = MyApplication.app.securityOptV2.dataEncrypt(2, dataIn, Security.DATA_MODE_ECB, null, dataOut);
            if (code < 0) {
    ...
    private void saveRSAKey() {
        try {
            ...
            byte[] dataOut = new byte[2048];
            int code = MyApplication.app.securityOptV2.saveRSAKey(0, pubKey, 2);
            if (code < 0) {
            ...
            code = MyApplication.app.securityOptV2.saveRSAKey(1, pvtKey, 3);
            if (code < 0) {
                LogUtil.e(TAG, "save RSA public key failed:" + code);
                return;
            }
            ...
            String data = "\"I have something to tell you, white mouse,\" she said. \"Mr. Behrman died of pneumonia to-day in the hospital."
                    ...
                    + " Didn't you wonder why it never fluttered or moved when the wind blew? Ah, darling, it's Behrman's masterpiece - he painted it"
                    + " there the night that the last leaf fell.";
            int len = MyApplication.app.securityOptV2.dataEncryptRSA(AidlConstantsV2.Security.RSA_TRANSFORMATION_4, 2, data.getBytes(), dataOut);
            if (len < 0) {
                LogUtil.e(TAG, "RSA private key encrypt data failed:" + len);
                return;
            }
            byte[] encryptOut = Arrays.copyOf(dataOut, len);
            len = MyApplication.app.securityOptV2.dataDecryptRSA(AidlConstantsV2.Security.RSA_TRANSFORMATION_4, 3, encryptOut, dataOut);
            if (len < 0) {
                LogUtil.e(TAG, "RSA private key decrypt data failed:" + len);
                return;
            }
